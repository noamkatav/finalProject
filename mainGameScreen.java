	...
	int lastScore =0;
		...
		if(((Math.sqrt((Math.abs(this.tfi-this.tli))^2 + (Math.abs(this.tfj-this.tlj))^2)) == 1)&&(CheckFreeSpace(tfi,tfj,tli,tlj)))
		{
			...
			//this.scores.setText(" The computer is now playing" +  " " + getScore().playerScore + " : "+ getScore().computerScore+ " " + theDepth);
				...
				{
				if((startingScreen.isTutorial)||(!colorPickerScreen.pvp.isSelected()))
				{
				kilo = bestPlace();
				this.tfi = kilo.fi;
				this.tfj = kilo.fj;
...
ActionListener actionListenerForMenu = new ActionListener() {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource().equals(restart)) {
			...
			new mainGameScreen(colorForPlayer,colorForAi,colorForSquers,colorForBackGround); 
...
ActionListener actionListener = new ActionListener() {
	...
	public static JLabel scores =  new JLabel();;
	 ...
	 public mainGameScreen(Color colorForPlayer, Color colorForAi,Color colorForSquers,Color colorForBackground)//constructor
	{
		...
		this.exit.addActionListener(actionListenerForMenu);
		this.restart.addActionListener(actionListenerForMenu);
		this.newGame.addActionListener(actionListenerForMenu);
